...
class APIRequests {
  Future firstSuperRequest(
    ...
    await requestUserOrders(userToken!, userId!, pageNumber, context,false).then(
        (value) =>
            context.read(generalmanagment).setTotalOrderPages(value.lastPage));

    await requestProducts(userToken, context, pageNumber, true).then((value) =>
        context
  ...
  Future requestUserOrders(String userToken, String userId, int pageNumber,
      BuildContext contextm, bool isRefresh) async {
    Dio dio = Dio();
    Options requestOptions = Options(
    ...
    if (isRefresh) {
      ...
      contextm.read(generalmanagment).orders=[];
    }
    try {
  ...
  Future requestProducts(String? userToken, BuildContext contextm,
    ...
    if (isRefresh) {
       // to be used later in refreshing the data if requested.
      contextm.read(generalmanagment).products=[];
    }
    try {
      var response = await dio.get(
